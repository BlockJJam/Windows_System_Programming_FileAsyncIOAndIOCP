...
VOID CALLBACK TimerAPCProc(PVOID pArgs, DWORD dwTimerLowValue, DWORD dwTimerHighValue)
{
	SYSTEMTIME st;
	GetLocalTime(&st);
...
void _tmain()
{
	...
	LARGE_INTEGER IUTC;
	IUTC.LowPart = ftUTC.dwLowDateTime;
	IUTC.HighPart = ftUTC.dwHighDateTime;

	int nSnoozeCnt = 0;
	SetWaitableTimer(hTimer, &IUTC, dwPeriod * 1000, TimerAPCProc, &nSnoozeCnt, TRUE);
