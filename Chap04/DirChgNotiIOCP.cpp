...
DWORD WINAPI DirMonProc(PVOID pParam)
{
	...
	while (true)
	{
		...
		(
			hDir, pdb->_buff, BUFF_SIZE, FALSE, DIR_NOTI_FILTER, NULL, pov, NULL
		);
		if (!bIsOK)
...
void _tmain(int argc, _TCHAR* argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		(
			argv[i], GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING,
		...
		(
			hDir, pdb->_buff, BUFF_SIZE, FALSE, DIR_NOTI_FILTER, NULL, pdb, NULL
		);
		if (!bIsOK)
		{
			cout << "ReadDirectoryChangesW failed: " << GetLastError() << endl;
