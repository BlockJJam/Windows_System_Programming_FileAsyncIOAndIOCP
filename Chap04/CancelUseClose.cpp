...
{-
	...
	while (true)
	{
		...
		(
			hDir, arBuff, BUFF_SIZE, FALSE, DIR_NOTI_FILTER, NULL, &ov, NULL
		);
		...
		if (ov.InternalHigh == 0 || ov.Internal != 0)
		{
			...
			if (dwErrCode == ERROR_NOTIFY_CLEANUP)
				cout << " <== Directory Monitoring stooping...." << endl;
			else
...
void _tmain()
{
	...
	(
		_T("C:\\Temp"), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
