...
void _tmain(int argc, _TCHAR* argv[])
{
	...
	(
		argv[1], GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL
	);
	...
	while (true)
	{
		...
		DWORD dwReadBytes = 0;
		blsOK = GetOverlappedResult(hSrcFile, &ro, &dwReadBytes, TRUE);
		if (!blsOK)
		{
			dwErrCode = GetLastError();
		...
		blsOK = GetOverlappedResult(hDstFile, &wo, &dwWroteBytes, TRUE);
		if (!blsOK)
		{
			dwErrCode = GetLastError();
			break;
